...
import type { ComplementaryConcept } from '../../../entities/student/api/enrollment';
...
export const AssignComplementaryModal: React.FC<AssignComplementaryModalProps> = ({
  isOpen,
  onClose,
  matriculaId,
  year,
  ...
  const fetchConcepts = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await enrollmentApi.getComplementaryConcepts(year);
      setConcepts(data);
      if (data.length > 0) {
  ...
  const handleCreateConcept = async (e: React.SyntheticEvent) => {
    ...
    try {
      ...
      const res = await enrollmentApi.createComplementaryConcept({
  ...
  const handleAssign = async (e: React.SyntheticEvent) => {
    ...
    try {
      ...
      if (targetConcept && descuento > targetConcept.valor) {
        throw new Error(`El descuento ($${descuento.toString()}) no puede superar el valor del concepto ($${targetConcept.valor.toString()}).`);
      }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={showCreateForm ? 'Crear Nuevo Concepto Complementario' : 'Asignar Concepto Complementario'} width={480}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        {!loading && !showCreateForm && (
          <form onSubmit={(e) => { void handleAssign(e); }} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        {!loading && showCreateForm && (
          <form onSubmit={(e) => { void handleCreateConcept(e); }} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <Input
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '4px 0' }}>
              <input
                ...
                onChange={(e) => { setNewConceptUsoMatricula(e.target.checked); }}
                disabled={createLoading}
                style={{ width: '16px', height: '16px', cursor: 'pointer', accentColor: 'var(--primary)' }}
              />
              <label htmlFor="usoMatriculaCheck" style={{ fontSize: '0.875rem', color: 'var(--text-main)', cursor: 'pointer', userSelect: 'none' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '12px' }}>
              <Button type="button" variant="secondary" onClick={() => { setShowCreateForm(false); }} disabled={createLoading}>
                Volver
              </Button>
